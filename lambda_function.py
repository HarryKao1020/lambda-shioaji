...
def get_account_info(event):
    ...
    # 判斷買賣別 (Action.Buy=多, Action.Sell=空)
    ...
    future_pos_df["direction_label"] = future_pos_df["direction"].apply(get_direction_label)

    # 計算曝險金額 (quantity 取絕對值, 方向由 direction_label 決定)
    future_pos_df["exposure"] = (
        future_pos_df["last_price"]
        * future_pos_df["quantity"].abs()
        * future_pos_df["multiplier"]
    )

    # 依方向拆分曝險
    long_exposure = future_pos_df.loc[
        future_pos_df["direction_label"] == "多", "exposure"
    ].sum()
    ...
    # 多空比率 (以曝險金額計算)
    if short_exposure > 0:
        long_short_ratio = f"{long_exposure / short_exposure:.2f}"
    elif long_exposure > 0:
        long_short_ratio = "純多單 (無空單)"
    else:
    ...
    result = {
        ...
        "多單曝險金額": f"{long_exposure:,.0f}元",
        "空單曝險金額": f"{short_exposure:,.0f}元",
        "總曝險金額": f"{total_exposure:,.0f}元",
        "淨曝險金額": f"{net_exposure:,.0f}元",
        "多空比率(多/空)": long_short_ratio,
